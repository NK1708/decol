...
class Woocommerce_Products_Per_Page {
	...
	public function load_textdomain() {
		...
		$locale = apply_filters( 'plugin_locale', get_locale(), 'patron' );

		// Load textdomain
		load_textdomain( 'patron', WP_LANG_DIR . '/woocommerce-products-per-page/woocommerce-products-per-page-' . $locale . '.mo' );
		load_plugin_textdomain( 'patron', false, basename( dirname( __FILE__ ) ) . '/languages' );
	...
	public function add_plugin_action_links( $links, $file ) {
		...
		if ( $file == plugin_basename( __FILE__ ) ) :
			$links = array_merge( array(
				'<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=products&section=display#s2id_wppp_dropdown_location' ) ) . '">' . __( 'Settings', 'patron' ) . '</a>'
			), $links );
		endif;
